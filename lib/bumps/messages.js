...
const updateBump = async (channel, messageId, memberId) => {
    const bumpEntry = await BumpsList.query({where: {message_id: messageId}, whereNull: 'member_id'}).fetch({withRelated: 'bump'});
    const message = await channel.fetchMessage(messageId);
    const {bump: {website, username, password}} = await bumpEntry.then(toJSON);
