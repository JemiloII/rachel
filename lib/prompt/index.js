...
let lastText;
const listener = client => async text => {
    logger.verbose('received data:', util.inspect(text));

    switch (true) {
        case text.startsWith('/link'):
            let link = await client.generateInvite(['ADMINISTRATOR']);
            return logger.info(link);
        ...
        case text.startsWith('/client'):
            ...
            console.log(input, '\n', properties);
            return logger.info(getProperty(client, properties));
        default:
            lastText = text;
...
const init = client => {
    if (process.env.NODE_ENV === 'development' && config.get('enable.prompt')) {
