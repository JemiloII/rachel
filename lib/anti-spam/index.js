...
const init = (client) => {
    ...
    client.on('message', async message => {
        ...
        const isBotSpam = /.*(naked|nude|.vip).*/gi.test(message.content);
    ...
    client.on('guildMemberAdd', async member => {
        logger.debug(`Checking if ${member.displayName} is a bot.`);
        const url = member.user.displayAvatarUrl;
        if (/^[A-Z]\w*(\.\w*){1,2}$/.test(member.displayName)) {
            logger.debug(`${member.displayName} matches xxx.xxx.xxx`);
            return await ban(member);
        }

        if (/^[A-Z]\w*\d{1,4}$/.test(member.displayName)) {
            logger.debug(`${member.displayName} matches Xxxxx1234`);
            if (isOverADayOld(member.user.createdAt)) {
                logger.debug(`${member.displayName} is over a day old.`);
                // return await ban(member);
            }

            if (!url) {
                logger.debug(`${member.displayName} has no avatar.`);
                return await ban(member);
            }

            if (await isAvatarNaked(url)) {
                logger.debug(`${member.displayName} has a naked avatar.`);
                return await ban(member);
            }
        }
